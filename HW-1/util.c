...
/*My multiply algorithm works for all 0 situations , negative positive situations and every number of digit.I know it is kind of long ,but everything works flawless for every situation.Not only for 3 digits , but also every number of digits.I know my code is kind of long ,but it works for every stiatuon.*/
...
void displaying_multiply(int n1 , int n2){
    ...
    if(result_multip<0){ /*I am drawing the division line right now.*/
        ...
        add_division_line(digit_result+4); /*No need one more space.There is no exception.*/
    }
  ...
  /* Until this phase , found number of digits of  n1 , n2 and result(n1*n2).Then I used as many spaces as the difference in the number of digits between result and n1 and n2.*/
    ...
    while(temp_n2!= 0){ /*This loop is gonna run as many as number of digits of n2*/
        ...
        remain =temp_n2%10; /*AS A RULE OF MULTIPLYING, EVERY DIGIT OF SECOND NUMBER IS MULTIPLIED BY FIRST NUMBER*/
        ...
        if(result_remain_multip ==0){  /*Bu kosul  sadece outputa 0 yazmak yerine n1 in basamk sayisi kadar 0 yazmasi icink kurulmus gorece extra bir kosul.Example  123
           ...
           int  space_of_result_remain =(digit_result)-(digit_n1)-k;
            ...
            if(n1 <0 || n2 <0){
                ...
                add_space(space_of_result_remain); /*No need for this conditon.*/
            ...
            int space_of_result_remain =(digit_result)-(digit_result_remain)-k; /*Do not forget to sub  'k'.It is the maain thing about swiping left in every result of remain multiply.*/
    ...
    if(result_multip<0 ||(n1<=0 && n2<=0)){ /*We must add 1 more line  because of minus '-' symbol.*/
        ...
        add_division_line(digit_result+4); /*No need it is not negative.*/
    ...
    if(result_multip==0){  /*All of these are because of 0 exceptation.*/
        ...
        if(n1 ==0 && n2 ==0){
            add_space(digit_n1+4-1); /*Same logic with the above.*/
            printf("%d",result_multip);
        }
...
int finding_digit(int number){
    ...
    if(number ==0){
        digit_number =1; /*0 is also 1 digit.Do not forget this condition.*/
    }
    while(number !=0){
        number = number/10;
...
void add_space(int space_of_number){
    printf("  "); /*I wanted to add 2 space because it will look better by  means of this.It just changes my code aesthetichly*/
...
void add_division_line(int digit_result){
    ...
    for(int i = 1; i<=digit_result ; i++){   /*Same process is also valid for this function.By means of that function , my length of divison line will look perfect.*/
